...
```
Bappaditya-AIML-Internship/
│
├── README.md                                  → This file
│
├── Report/
│   └── Internship_Report_Bappaditya_Das.pdf   → Full internship report (university format)
│
├── Presentation/
│   └── AI_ML_Internship_Presentation.pdf      → Viva / evaluation presentation deck
│
└── Certificates/
    ├── Offer_Letter.pdf                       → Internship offer letter (SmartED Innovations)
    └── Completion_Certificate.pdf             → Certificate of Internship Completion
```

> All four artifacts — the report, the presentation, the offer letter, and the completion certificate — are version-consistent and cross-referenced with each other (e.g., the report's Annexure section reproduces both certificates in full).
...
**1. NYC Room Classification**
Given a dataset of room/property listings from New York City — with attributes such as location (borough/neighbourhood), room type, price, minimum nights, and availability — the task was to build a **classification model** capable of categorising listings (e.g., by room type or price category).

**2. Mental Health Score Prediction**
Given a dataset of lifestyle and behavioural indicators (sleep patterns, screen time, physical activity, stress-related factors), the task was to build a **regression model** capable of estimating a continuous Mental Health Score and identifying the factors that influence it most.
...
|---|---|
| Internship Report | Complete report following the university-prescribed format — Candidate's Declaration, Acknowledgement, Table of Contents, List of Tables/Figures, Project Description, Training Log Book, and Bibliography. |
| Presentation | Slide deck summarising the internship, organization, problem statements, architecture, methodology, and outcomes, for viva/evaluation. |
| Offer Letter | Official virtual internship offer issued by SmartED Innovations. |
| Completion Certificate | Certificate of Internship Completion issued by SmartED Innovations, verifying successful completion of the program. |
